...
- [x] 拡大表示ボタン
  - シームレスモード時のみ表示
  - クリックで「拡大表示」⇔「標準表示」を切り替え
- [x] 2倍拡大表示
  - 画像が2倍のサイズで表示
  - 横読みモード・縦読みモードの両方に対応
- [x] 拡大時の両方向スクロール
  - 横読みモード + 拡大時：横方向と縦方向の両方にスクロール可能
  - 縦読みモード + 拡大時：縦方向と横方向の両方にスクロール可能
- [x] スムーズなドラッグスクロール
  - ドラッグ中は scroll-behavior: auto で即座に反映
  - ドラッグ終了時は scroll-behavior: smooth に戻す
- [x] スクロール位置の自動調整
  - 拡大/縮小時にスクロール位置を倍率に応じて調整
...
- **作業状況**: スプラッシュ画面と拡大表示機能の実装完了

### 本セッションで実装した機能
1. **スプラッシュ画面**
   - ロゴ画像（public/images/mukuviewer.png）を中央に白背景で表示
   - パルスアニメーション（拡大縮小）
   - 初期化完了後0.5秒待機してフェードアウト（0.8秒）

2. **拡大表示機能（シームレスモード）**
   - 拡大表示ボタンの追加（シームレス時のみ表示）
   - 2倍の拡大倍率（ZOOM_SCALE = 2）
   - 横読みモード・縦読みモードの両方に対応
   - 拡大時は両方向にドラッグスクロール可能
     - 横読みモード + 拡大時：横方向と縦方向の両方にスクロール
     - 縦読みモード + 拡大時：縦方向と横方向の両方にスクロール
   - スムーズなドラッグスクロール（scroll-behavior制御）
   - スクロール位置の自動調整（拡大/縮小時）

3. **技術的改善**
   - `scroll-behavior`の動的制御（ドラッグ中は`auto`、それ以外は`smooth`）
   - ドラッグスクロール関数の変数スコープ修正
   - 拡大時のCanvas再作成とリサイズ処理
