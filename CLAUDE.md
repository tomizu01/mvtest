...
- **横長画面**: 見開き2ページ表示（右めくり方式）
  - 1ページ目（表紙）は画面中央に単独表示
  - 2ページ目以降は見開き表示（2-3, 4-5, 6-7...）
  - 左右ボタンでページ移動（表紙から次は1ページ、以降は2ページずつ）
- **縦長画面**: 1ページずつ表示
  - 画面いっぱいに1ページを表示
  - 左右ボタンで1ページずつ移動
...
#### シームレスモード
- **横読みモード**（横長画面で自動選択）
  - 右から左へ横スクロール（19, 18, 17... 2, 1）
  - ドラッグまたは自動再生でスクロール
- **縦読みモード**（縦長画面で自動選択）
  - 上から下へ縦スクロール（1, 2, 3... 17, 18, 19）
  - 1ページを画面幅いっぱいに表示
  - ドラッグまたは自動再生でスクロール
- 画面の向きが変わると自動的に切り替わる
...
### 2. シームレス表示モード
- [x] 横読みモード
  - 右から左への横スクロール（降順：19, 18, 17... 2, 1）
  - マウス/タッチドラッグでスクロール
  - 動的ページ読み込み（最大19ページ）
  - ビューポートベースの描画最適化
- [x] 縦読みモード
  - 上から下への縦スクロール（昇順：1, 2, 3... 17, 18, 19）
  - 1ページを画面幅いっぱいに表示
  - マウス/タッチドラッグでスクロール
  - 動的ページ読み込み（最大19ページ）
- [x] 自動再生機能
...
├── images/
│   └── 00000001/           # テスト用画像 (1.jpg.enc～209.jpg.enc、デモ版では19ページまで利用)
├── public/
│   └── images/
...
const int maxPages = 19;              // 最大ページ数（デモ版）
```
...
- **作業状況**:
  1. 表紙（1ページ目）の単独中央表示機能を実装
  2. 最大ページ数を20から19に変更

### 過去のセッションで実装した機能（2025-11-18）
...
25. ✅ 表紙単独表示機能 → 完了（1ページ目を中央に単独表示）

### 本セッションで実装した機能（2025-12-05）

#### 1. 表紙（1ページ目）の単独中央表示機能
**目的**: 見開きモード（横長画面）で、1ページ目（表紙）を画面中央に単独表示する

**実装内容**（main.cpp）:
1. **`isCoverPage()`関数を追加**（82-84行目）
   ```cpp
   bool isCoverPage() const {
       return !singlePageMode && currentPage == 1;
   }
   ```
   - 横長画面（見開きモード）で1ページ目にいる場合にtrueを返す

2. **`loadCurrentPages()`を修正**（86-98行目）
   - 表紙の場合は1ページのみ読み込み

3. **`checkAndRender()`を修正**（271行目）
   - 表紙の場合は1ページモードと同様に判定

4. **`renderPages()`を修正**（287-402行目）
   - 表紙の場合は1ページのみを中央に描画

5. **`nextPage()`を修正**（449-470行目）
   - 表紙（1ページ目）からは1ページだけ進む（→2ページ目へ）
   - 2ページ目以降は2ページずつ進む

6. **`prevPage()`を修正**（473-497行目）
   - 2ページ目から戻る場合は1ページだけ戻る（→表紙へ）
   - それ以外は2ページずつ戻る

**ページ遷移の例（横長画面・見開きモード）**:
- 初期表示: 1ページ目（表紙）が画面中央に単独表示
- 次へ: 2-3ページが見開き表示
- 次へ: 4-5ページが見開き表示
- ...
- 次へ: 18-19ページが見開き表示（最後）

**注意**: 縦長画面（1ページモード）とシームレスモードの動作は従来通り

#### 2. 最大ページ数を20から19に変更
**目的**: 表紙単独表示により最後が単一ページになるのを防ぐ

**変更箇所**:
- main.cpp（59行目）: `maxPages` を20から19に変更
- js/main.js（255行目）: `MAX_PAGES` を20から19に変更
- シームレスモード初期化のループを修正（521-525行目、548-552行目）

**効果**:
- 見開きモードでのページ遷移: 1（表紙）→ 2-3 → 4-5 → ... → 18-19（最後）
- 最後のページが見開きで完結するようになった

### 過去のセッションで実装した機能（2025-12-02）
