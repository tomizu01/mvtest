...
void Cipher16(uint8_t* data,uint8_t* key){
    int i;
    AddRoundKey(data,0);
    for(i=1;i<10;i++){
        SubBytes(data);
...
int Cipher( uint8_t* data, uint8_t* key, int size )
{
    int i, j;
    int enc_size;
    ...
    for( i = 0 ; i <= size / 16 ; i ++ ){
        if( i == size / 16 ){
            int padding;
            padding = i * 16 + 16 - size;
            for( j = 0 ; j < padding ; j++ ){
                data[i * 16 + 15 - j] = padding;
            }
        }
        Cipher16( data + i * 16, key);
        enc_size += 16;
    }
    return enc_size;
}
 
void invCipher16(uint8_t* data,uint8_t* key){
    int i;
    AddRoundKey(data,10);
    for(i=9;i>0;i--){
        invShiftRows(data);
...
int invCipher( uint8_t* data, uint8_t* key, int size )
{
    int i, j;
    int dec_size;
    int padding_size;
    ...
    dec_size = 0;
    for( i = 0 ; i < size / 16 ; i ++ ){
        invCipher16( data + i * 16, key );

        if( i  == ( size / 16 ) - 1 ){
            padding_size = data[i*16+15];
            dec_size += ( 16 - padding_size );
        }
        else{
            dec_size += 16;
        }
...
int main(int argc,char** argv){
    ...
    int s,size;
    uint8_t data[16];
    static uint8_t buf[1000000000];
    int i,j;
    char filename1[256];
    char filename2[256];

    if( argc <= 2 ){
        printf("usage c [filename] [enc/dec]\n");
        exit(1);
    }
    ...
    strcat( filename1, ".enc" );
    strcpy( filename2, argv[1] );
    strcat( filename2, ".dec" );

if( strcmp( argv[2], "enc" ) == 0 ){

    fp = fopen( argv[1],"rb");
    size = 0;
    while( ( s = fread( buf + size , 1, 16, fp ) ) > 0 ){
        size += s;
    }
    fclose( fp );

    size = Cipher(buf,key,size);

    fpw = fopen( filename1,"wb");

    for( i = 0 ; i < size ; i += 16 ){
        fwrite( buf + i, 1, 16, fpw );
    }
    fclose( fpw );
}
    ...
    size = invCipher(buf,key,size);

    fpw = fopen( filename2,"wb");
    for( i = 0 ; i < size ; i += 16 ){
        if( i + 16 >= size ){
            int sz = 16 - ( i + 16 - size );
            fwrite( buf + i, 1, sz, fpw );
        }
        else{
            fwrite( buf + i, 1, 16, fpw );
        }
